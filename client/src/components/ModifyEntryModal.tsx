...
function ModifyEntryModal({
  ...
  const mutation = useMutation(
    (openType === 'create'
      ? forgeAPI.modrinth.entries.create
      : forgeAPI.modrinth.entries.update.input({
          id: initialData!.id
        })
    ).mutationOptions({
      onSuccess: () => {
        queryClient.invalidateQueries({ queryKey: ['modrinth', 'entries'] })
      },
      onError: error => {
  ...
  const { formProps } = defineForm<
    InferInput<(typeof forgeAPI.modrinth.entries)['create' | 'update']>['body']
  >({
    onClose,
